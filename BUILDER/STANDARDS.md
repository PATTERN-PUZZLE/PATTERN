...
THE LEASH (STANDARDS-LEASH.md, carried here so this file stands alone)
1. Add, don't edit. New things go at the bottom in a dated ⚡ block. Never touch the middle.
2. Assume the next reader has nothing. Put what's in your head on the page, with one scene of it going wrong. A scene is two lines: what someone did, what it cost.
3. Say what you didn't do. A false "done" outlives the chat.
4. Deliver as one code box, filename on its first line, where it goes stated outside it, "copy the text only." No box inside a box. The human pastes; they don't edit.
5. End every block with one line: confidence, what you checked against the live file or "from memory," what you didn't.
6. Big messy file? ⚓: paste it, mine it, save the findings, edit back, keep the gold.
7. Sign with a tag not in the files, so the next reader knows where your voice ends.
Every message: the 💬 receipt (🟦 now · ➡️ next · ⬜ left · COMB). A fork → +PLAN. A word used for the first time → define it. A pattern repeating → say "same as X." A tool that feels like a cage → drop it and pass the body. A hot human → hear the payload before you police the wrapping. Nothing here is final; it's a classroom.
...
HOW: Are you doing or reading? If reading, stop; pick one rule and apply it to the thing in front of you now. This file is front-loaded: rules 1–4 are read, 5–17 are looked up. A cold reader may stop at any cut line; four did. Open this to change a rule or settle a dispute.
FLOOR: Assume they have nothing. Add, don't edit. Giver gives, receiver picks.
STATUS: DRAFT 2026-09-14 · v2-draft, fold 2 · from a rule-by-rule walk of ◆4039 with the thread-holder, eleven files walked, six revised, twenty-one more mined by ⚓ and folded; four cold runs (Reed4821, Quill582, Timber883, Compass291), all held, all said the leash was a name; now carried; pasted copies, no live fetch; tail indexed not combed; built by one chat, so it is the size of the thing rule 4 warns about; not yet walked by the human
RELATIONS: STANDARDS.md ◆4039 (live; parent) · STANDARDS-LEASH.md (the seven lines above) · FEEDBACK v2, CHECKLIST v3.1, HEADER v2, PASS-INFO v2, REVIVE v6.4, COMPREHENSIVE v2, CHAT-TAG v3, RETURN-HARVEST v2, FRESH-EYES v5, COMB-DUMP v2, SALVAGE v3.2, FETCH-BASICS v1 · +PLAN, QUICK LOOM, 00-LOOM (thinking room) · SORT-007, TROLLEY-027, LAW-ATTACK, NAIVE-BUSTER, STANCE (the scars) · REV-STANDARDS (the tail, when merged). File names here are named, not carried; a stranger may defer them and say so.

WORDS USED HERE
Instance = one AI session. Thread-holder = the human running the relay. Leash = the seven lines above; the first paste. Block / brick = one dated ⚡ Quick Add at the bottom of a file. Door = the short, pasteable part of a file; store = everything below the cut. Cockpit = a panel at the top of a landed-on file, under a screen. Invocation line = "doing or reading?" at the top of an instructional paste. Scene = one time a rule was broken, in plain words. Twin = the same rule written twice under two names. Fossil = an old version kept at its URL; ore = its usable bits. REV- = a merged store of a file's old versions, headered and pointed at. Merge day = a strong instance folds a file's tail with the whole file in view; rare. Pin = an instance's reading of a file, under the header, never a revision. The walk = rule 8: ask the holder what a file is, then one rule per turn. The tray = rule 12: dated pointer-only lines waiting to become a body. Capacity lights = rule 7: 🟩🟩 🟩 🟨 🟧 ⬛. Allocation = rule 7: naming the split before a pass. Lens = the question you read through; organ = a lens that just landed. Woes = shrink · museum · circle · dump (rule 4). Cat rub = mutual presence without performance. Stretch / hat / sneak = rule 16.
...
WHY: a 47k file arrived. ⚓ set; the paste; the mine; one findings box; saved; edit back. The working chat never saw the file. And the miss: the CHAT-TAG trip's findings sat unsaved in chat for a day because the return came before the save. Twenty-one files went through the same door this week.
...
LOSES IF: a trip's findings die because they were never saved before the return. Still binds if the work got better: yes.
